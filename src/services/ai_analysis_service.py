"""
AI分析服务
使用OpenRouter API调用Qwen模型进行视频内容与封面分析
"""
import json
import requests
...
class DoubaoService:
    """OpenRouter API服务（使用Qwen模型）"""

    BASE_URL = "https://openrouter.ai/api/v1"
    MODEL = "qwen/qwen3.7-flash"

    def __init__(self, api_key: Optional[str] = None, model: str = "qwen/qwen3.7-flash"):
        # 优先使用传入的 api_key，其次从数据库读取，最后使用环境变量
        self.api_key = api_key or _get_ai_api_key_from_db() or settings.OPENROUTER_API_KEY
        self.model = model
    ...
    def _call_api(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        headers = {
            ...
            "HTTP-Referer": "https://meng-yuan.uk",
            "X-Title": "BOMBO",
        }
    ...
    def _call_multimodal_api(self, text_prompt: str, image_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            import os
            # 避免 SSL 证书问题
            os.environ['SSL_CERT_FILE'] = os.environ.get('SSL_CERT_FILE', '/etc/ssl/certs/ca-certificates.crt')
            ...
            print(f"OpenRouter Multimodal API call failed: {e}")
            return None
...
class AIAnalysisService:
    ...
    def cache_analysis(self, result: AIAnalysisResult, title: Optional[str] = None, cover_url: Optional[str] = None) -> bool:
        """缓存分析结果到数据库（永久有效）"""
        try:
            with get_db_session() as session:
                # 缓存封面分析结果
                if result.cover_analysis:
                    session.execute(
                        text("""
                            INSERT INTO ai_cache (bvid, analysis_type, title, cover_url, result_data)
                            VALUES (:bvid, :analysis_type, :title, :cover_url, :result_data)
    ...
    def batch_analyze(
        ...
        for video in videos:
            ...
            if analysis:
                # 缓存结果
                self.cache_analysis(analysis, title=video.title, cover_url=cover_url)
